...
export default function CategoryCard({ item }) {
    return (
        <Link
            ...
            className="relative flex h-80 w-full flex-col rounded-lg p-9 hover:opacity-75 xl:w-auto"
        >
            <span className="absolute inset-0">
                <img src={item.cover} alt={item.name} className="h-full w-full object-cover object-top" />
            </span>
            <span className="absolute inset-x-0 bottom-0 h-2/3 bg-gradient-to-r from-slate-800 opacity-50" />
