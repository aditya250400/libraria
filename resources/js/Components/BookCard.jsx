    ...
    return (
        <div className="group relative">
            <div className="relative overflow-hidden rounded-lg bg-muted/40">
                ...
                <div className="absolute inset-0 flex items-end p-4 opacity-0 transition-all duration-200 ease-in-out group-hover:opacity-100">
                    <Link
                        href={route('front.books.show', [item.slug])}
                        className="w-full rounded-md bg-white bg-opacity-75 px-4 py-2 text-center text-sm font-medium text-foreground backdrop-blur backdrop-filter"
                    >
                        Lihat Buku
                    </Link>
                </div>
